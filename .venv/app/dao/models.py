...
class User(Base):
    __tablename__ = "users" # фундаментальный элемент SQLAlchemy‑модели, связывающий Python‑класс с таблицей БД
...
class TimeSlot(Base):
    ...
    bookings: Mapped[list["Booking"]] = relationship(
        "Booking",
        back_populates="time_slot",  # в SQLAlchemy задаёт двунаправленную связь между двумя ORM‑моделями
        cascade="all, delete-orphan" # Если дочерний объект отсоединился от родителя (т. е. стал «потерянным», orphan), он будет автоматически удалён из БД.
    )
