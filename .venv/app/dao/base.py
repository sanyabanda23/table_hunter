...
# Объявляем типовую переменную с ограничением
T = TypeVar("T", bound=Base)


class BaseDAO(Generic[T]):
    model: Type[T] = None  # Здесь гарантированно имеет интерфейс Base
    ...
    # Формирование запроса на поиск записи по id
    async def find_one_or_none_by_id(self, data_id: int):
        try:
            query = select(self.model).filter_by(id=data_id) # Создаёт SQL‑запрос SELECT c eckjdbtv WHERE
            result = await self._session.execute(query) # Метод для выполнения SQL‑запроса
            record = result.scalar_one_or_none() # для извлечения одной скалярной величины из результата запроса
    ...
    # Формирование запроса на поиск 1 записи по данным по фильтру
    ...
    # Формирование запроса на поиск всех записи по данным по фильтру
    async def find_all(self, filters: BaseModel | None = None):
        ...
        try:
            query = select(self.model).filter_by(**filter_dict)
            result = await self._session.execute(query)
            records = result.scalars().all() # Преобразует сырые строки (Row) в экземпляры модели ORM и собирает все экземпляры в список
    ...
    # Добавление записи с параметрами values
    async def add(self, values: BaseModel):
        ...
        try:
            new_instance = self.model(**values_dict) # создаёт экземпляр модели SQLAlchemy на основе переданного словаря данных
            self._session.add(new_instance) # добавляет экземпляр модели в сессию для последующего сохранения в базе данных
            logger.info(f"Запись {self.model.__name__} успешно добавлена.")
            await self._session.flush() # Формирует и выполняет INSERT, UPDATE, DELETE для всех накопленных изменений
            return new_instance
        except SQLAlchemyError as e:
            logger.error(f"Ошибка при добавлении записи: {e}")
            raise

    # Добавление несколькиих записей по списку с параметрами values
    ...
    async def update(self, filters: BaseModel, values: BaseModel):
        ...
        try:
            ...
            await self._session.flush()
            return result.rowcount # возвращает количество строк, затронутых выполнением SQL‑запроса
        except SQLAlchemyError as e:
