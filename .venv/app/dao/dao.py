    ...
    async def book_count(self) -> Dict[str, int]:
        ...
        try:
            ...
            for status in statuses:
                ...
                result = await self._session.execute(query)
                count = result.scalar() # для получения одного значения из первой строки результата запроса.
                status_counts[status] = count
                logger.info(f"Найдено {count} заявок со статусом '{status}'.")

            total_query = select(func.count(self.model.id))
            total_result = await self._session.execute(total_query)
            total_count = total_result.scalar() # для получения одного значения из первой строки результата запроса.
